...
# loop over kubernetes labels, store them as local variables
kubeLabelsLocation="/etc/kubernetes/labels";

cat /etc/kubernetes/labels
if [  -f $kubeLabelsLocation ]; then
        while read line; do
                ...
                k=${k//-/_};
                declare "${k}"=${v:1:${#v}-2}
        done < $kubeLabelsLocation
else
        $verbose && echo "Kubernetes labels mount not found at $kubeLabelsLocation, exiting now!" 2>&1 | tee -a /usr/src/app/run.log | tee >(logger)
        exit;
fi

if [ -z "$dpdkr" ]; then
        $verbose && echo "Variable \$dpdkr is not set, exiting now!" 2>&1 | tee -a /usr/src/app/run.log | tee >(logger)
        exit;
fi

# number of cores in this system (including multi-socket, multi-core, multi-threaded system)
coreCount=$(grep -c ^processor /proc/cpuinfo)
# core number this container will use
coreNum=$((dpdkr%(coreCount-1)+1))

# set MEMPOOL prefix & size
export mempoolprefix=ovs_mp_1500_0_;
export mempoolsize=262144;
